...
# the uncompressed vim.log files are way too large to keep around (tch April 2011)
  ...
  if /\.gz/ =~ _line[-1] && 
     /evm|production|top_output|mongrel\.3000|vmstat_output|serverlog|postgresql|audit|automation|policy|fog|aws|rhevm|scvmm/ =~ _line[-1] then
...
_temp_array =[$evm,$production,$top_output,$mongrel,$vmstat,$vim,$postgresql,$serverlog,$audit,$automation,$policy,$fog,$aws,$rhevm,$scvmm]
...
##     # create a new directory for each of the appliance names
     ...
     # using 7-Zip command
     selected_files = ""
     ["evm","production","top_output","mongrel.3000","vmstat_output","vim","postgresql","serverlog","audit","automation","policy","fog","aws","rhevm","scvmm"].each do |type|
         case type
...
#     end
          ["evm","production","top_output","mongrel.3000","vmstat","vim","serverlog","postgresql","audit","automation","policy","fog","aws","rhevm","scvmm"].each do |_y|
          case _y
            when "audit" then
              if $audit.size > 0 then
                rename_audit = `rename audit.log audit_output_last.log`
                ...
                File.delete("audit_output_last.log") if File.exists?("audit_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end
            when "automation" then
              if $automation.size > 0 then
                rename_automation = `rename automation.log automation_output_last.log`
                ...
                File.delete("automation_output_last.log") if File.exists?("automation_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end
            when "policy" then
              if $policy.size > 0 then
                rename_policy = `rename policy.log policy_output_last.log`
                ...
                File.delete("policy_output_last.log") if File.exists?("policy_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end
            when "fog" then
              if $fog.size > 0 then
                rename_fog = `rename fog.log fog_output_last.log`
                ...
                File.delete("fog_output_last.log") if File.exists?("fog_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end
            when "aws" then
              if $aws.size > 0 then
                rename_aws = `rename aws.log aws_output_last.log`
                ...
                File.delete("aws_output_last.log") if File.exists?("aws_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end
            when "rhevm" then
              if $rhevm.size > 0 then
                rename_rhevm = `rename rhevm.log rhevm_output_last.log`
                ...
                File.delete("rhevm_output_last.log") if File.exists?("rhevm_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end
            when "scvmm" then
              if $scvmm.size > 0 then
                rename_scvmm = `rename scvmm.log scvmm_output_last.log`
                ...
                File.delete("scvmm_output_last.log") if File.exists?("scvmm_output_last.log")
              else
                # do nothing if there are no log rotate gz files
              end              
              when "vmstat" then
